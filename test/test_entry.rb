...
class TestEntry < BeerTest
  def test_to_s_prints_details
    entries = Entries.new(name: "FooBeer", type: "porter", ounces: "20", cost: "10.00")
    expected = "FooBeer: porter type, 20 oz, $10.00, id: #{entries.id}"
    assert_equal expected, entries.to_s
  end

 def test_update_doesnt_insert_new_row
    entries = Entries.create(name: "Foo", type: "stout", ounces: "12", cost: "5.50")
  ...
  def test_update_saves_to_the_database
    entries = Entries.create(name: "Foo", type: "stout", ounces: "12", cost: "5.50")
    id = entries.id
    entries.update(name: "Bar", type: "stout", ounces: "6", cost: "10.50")
    updated_entries = Entries.find(id)
    expected = ["Bar", "stout", 6, "10.50" ]
    actual = [ updated_entries.name, updated_entries.type, updated_entries.ounces, updated_entries.cost]
    assert_equal expected, actual
  end

  def test_update_is_reflected_in_existing_instance
    entries = Entries.create(name: "Foo", type: "stout", ounces: "12", cost: "5.50")
    entries.update(name: "Bar", type: "stout", ounces: "20", cost: "10.50")
    expected = ["Bar", "stout", 20, "10.50" ]
    actual = [ entries.name, entries.type, entries.ounces, entries.cost]
    assert_equal expected, actual
  end


  def test_saved_entries_are_saved
    entries = Entries.new(name: "Foo", type: "pilsner", ounces: "12", cost: "5.50")
    foos_before = database.execute("select count(id) from entries")[0][0]
    entries.save
    foos_after = database.execute("select count(id) from entries")[0][0]
  ...
  def test_save_creates_an_id
    entries = Entries.create(name: "Foo", type: "pilsner", ounces: "12", cost: "5.50")
  ...
  def test_search_returns_entries_objects
    ...
    Entries.create(name: "Guinness Draught", type: "pilsner", ounces: "12", cost: "5.50")
  ...
  def test_search_returns_appropriate_results
    entry1 = Entries.create(name: "Foo", type: "pilsner", ounces: "12", cost: "5.50")
    entry2 = Entries.create(name: "Guinness", type: "pilsner", ounces: "12", cost: "5.50")
    entry3 = Entries.create(name: "Guinness Draught", type: "pilsner", ounces: "12", cost: "5.50")
  ...
  def test_all_returns_all_entries_in_alphabetical_order
    Entries.create(name: "foo", type: "stout", ounces: "12", cost: "5.50")
    Entries.create(name: "bar", type: "stout", ounces: "12", cost: "5.50")
    results = Entries.all
    expected = ["bar", "foo"]
 ...
 def test_equality_on_same_object
  ...
  def test_equality_with_different_entries
    entries1 = Entries.create(name: "foo", type: "stout", ounces: "12", cost: "5.50")
    entries2 = Entries.create(name: "bar", type: "stout", ounces: "12", cost: "5.50")
